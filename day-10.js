...
const instructions = [...(
	...
	.map(row => {
		// each instruction is parsed to an object of its lights, buttons, and joltages
		return {
			// "lights" is an array of 0 for unlit bulbs and 1 for lit bulbs
			"lights": row[1].split(``).map(bulb => bulb === `#` ? 1 : 0),
			// "buttons" is an array of arrays, one for each button,
			// where each button array is 1 if the button toggles the bulb at that index
			// or 0 if it doesn't toggle the bulb at that index
			"buttons": row[2]
				.slice(1, -1)
				.split(`) (`)
				.map(button => {
					const indices = button.split(`,`).map(n => parseInt(n));
					const buttonI = Array(row[1].length).fill(0);
					for (let i = 0; i < indices.length; i++) buttonI[indices[i]] = 1;
					return buttonI;
				}),
			// "joltages" is an array of the joltage numbers as numbers rather than text
			"joltages": row[3].split(`,`).map(n => parseInt(n)),
		};
	});
...
for (const instruction of instructions) {
	// initialise with the highest number of button presses
	let n = instruction.buttons.length;

	// the maximum number of combinations is 2 ** instruction.buttons.length
	// so we can convert that to a binary number, then a string, then an array of 0 and 1
	for (let i = 0; i < 2 ** instruction.buttons.length; i++) {
		// array of 0 if a button should be pressed, 1 if not
		const presses = i
			.toString(2)
			.padStart(instruction.buttons.length, `0`) // pad the start with 0s to make sure we get the right number
			.split(``)
			.map(n => parseInt(n));

		// 0-initialised array of the lights' state as each required button is pressed
		const state = Array(instruction.lights.length).fill(0);

		for (let button = 0; button < presses.length; button++) {
			// ignore buttons that aren't pressed
			if (presses[button] === 0) continue;

			// add up the toggle states of all the pressed buttons
			for (let b = 0; b < instruction.lights.length; b++) {
				state[b] += instruction.buttons[button][b];
			}
		}

		// if the final toggled state matches the required lights state...
		if (state.every((s, l) => s % 2 === instruction.lights[l])) {
			// ...take the min of the current n
			// and the number of presses for the current test
			n = Math.min(n, presses.filter(p => p === 1).length);
		}
	}
...
console.log(`The total number of button-presses is ${total}.`); // Answer: 434
